...
const COMMIT_HEADER_REGEX = /^(?:[\p{Emoji}\u200D\uFE0F]+\s*)?(?:\[AI\]\s*)?(feat|fix|docs|style|refactor|perf|test|chore|ci|build|revert|wip|ai)(?:\([a-z0-9._/-]+\))?!?: .+/u;
...
function inferCommitType(files) {
	...
	if (filePaths.some(f => f.includes('package.json') || f.includes('tsconfig') || f.includes('webpack') || f.includes('vite') || f.includes('Dockerfile') || f.includes('.yml') || f.includes('.yaml'))) {
		return 'chore';
	}

	// Check for styles
	if (filePaths.some(f => f.endsWith('.css') || f.endsWith('.scss') || f.endsWith('.less') || f.endsWith('.styl'))) {
		return 'style';
	}
...
function generateCommitMessage(options, aiData, gitInfo) {
	const { type = inferCommitType(getChangedFilesInfo()), scope = '', message = '', body = '', breaking = '', includeAI = true } = options;
	...
	const header = `${emoji} ${aiTag}${type}${scopeStr}${breakingMarker}: ${typeChinese} - ${message}`;
...
function installHook(options = {}) {
	...
	const entryScript = toHookScriptPath(path.join(__dirname, 'git-commit-ai.cjs'));
...
function validateAIInfoBlock(message) {
	const requiredRules = [
		{ key: 'Agents', rule: /^ {2}Agents: .+$/m },
		{ key: 'Sessions', rule: /^ {2}Sessions: \d+$/m },
		{ key: 'Involvement', rule: /^ {2}Involvement: (100|[1-9]?\d)%$/m },
		{ key: 'Depth', rule: /^ {2}Depth: (None|Low|Medium|High|Full)\b/m }
	];

	const missing = requiredRules.filter(({ rule }) => !rule.test(message)).map(({ key }) => key);
...
async function main() {
	...
	// Execute git commit
	try {
		const commitFile = path.join(require('node:os').tmpdir(), `git-commit-ai-${Date.now()}.txt`);
